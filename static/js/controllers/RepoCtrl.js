...
angular.module('app.controllers').controller('RepoCtrl', function ($scope, $http) {
    ...
    */
    ...
    $scope.tissueDisabled = function(name){
      models_available = Object.keys($scope.rx[name]);
      for(var i = 0;i < $scope.datasets.length;i++){        
        // if($scope.datasets[i]selected && models_available.indexOf($scope.datasets[i])){
        //   return true;
        // }
      }
      return false
    ...
    $scope.datasets = [
      {name: "Gabriel", model: "Cole-Cole", selected: false},
      {name: "Krewer", model:"Debye", selected: false}
    ];
    ...
    /*
      watch which variables the user selects to check for acceptable combinations
    */
    $scope.$watch('selected', function (newValue, oldValue) {
      ...
      // gabriel has been selected, check that all tissues support it
      if(newValue.poles.Krewer != oldValue.poles.Krewer){
        debug("krewer poles changed");
        for(var i = 0;i<$scope.tissues_sorted.length;i++){
          if($scope.tissues[$scope.tissues_sorted[i]].Krewer.poles_available.indexOf(newValue.poles.Krewer) == -1){
    ...
    */
    function parse_settings() {
      ...
      var tissues = [], datasets = [], freq = [];
      for(var j = 0;j < $scope.rx.length;j++){
        if($scope.rx[j].selected === true){
          tissues.push($scope.rx[j]._id);
        }
      }
      tissues = tissues.join(",");
      for(var i = 0; i < $scope.datasets.length;i++){
        if($scope.datasets[i].selected === true){
            datasets.push($scope.datasets[i].name);
          }
      }
    ...
    function plotKrewer(freq,data,poles){ 
        ...
        for(var i = 0; i < selected_tissues.length;i++){
          ...
          for(var j = 0; j < params.datasets.length;j++){
            ...
            switch(params.datasets[j]){
              case "Krewer":
                output[selected_tissues[i].name][params.datasets[j]] = plotKrewer(params.freq,selected_tissues[i].Krewer);
  ...
  */
   ...
   $scope.exportData = function() {
      ...
      */
      ...
      var names = Object.keys($scope.tissues) ;
      for(var i = 0; i < names.length;i++){
        if($scope.tissues[i].selected == true){
          postData.tissues.push($scope.rx[i]._id);
        }
      }

      for(var i = 0; i < $scope.datasets.length;i++){
        if($scope.datasets[i].selected == true){
          postData.models.push($scope.datasets[i].name);
        }
      }
      debug("post data");
      debug(postData);
